...
await tab.waitForSelector('.administration .active a', {visible:true});
await tab.click('.administration .active a')
let allselector = await tab.$$('.administration li a');
let sel=allselector[1];
await sel.click();
// await tab.waitForSelector('.btn.btn-green.backbone.pull-right', {visible:true});
// await tab.click('.btn.btn-green.backbone.pull-right');
await tab.waitForTimeout(3000)
let createchallengehref=await tab.$('.btn.btn-green.backbone.pull-right');
// await tab.waitForTimeout(3000)
...
await addchallenge(browser,link,i);
}
})();

async function addchallenge(browser , link,i){
...
await newtab.waitForTimeout(3000)
await newtab.type("#problem_statement-container .CodeMirror.cm-s-default.CodeMirror-wrap textarea",challenge["Problem Statement"])
await newtab.type("#input_format-container .CodeMirror.cm-s-default.CodeMirror-wrap textarea",challenge["Input Format"])
await newtab.type("#constraints-container .CodeMirror.cm-s-default.CodeMirror-wrap textarea",challenge["Constraints"])
await newtab.type("#output_format-container .CodeMirror.cm-s-default.CodeMirror-wrap textarea",challenge["Output Format"])
